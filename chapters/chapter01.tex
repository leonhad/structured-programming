...
$$
\text{\textbf{for} } 0 \leqslant i < 1000 : 0 \leqslant fx(i) < 100 \text{ \textbf{and} } 0 \leqslant fy(i) < 50.
$$
...
$$
x = fx(i) \text{ \textbf{and} } y = fy(i) \text{ for some } i \text{ satisfying } 0 \leqslant i < 1000
$$
...
\begin{enumerate}[label=(\arabic*)]
	\item that the operation ``build'' assigns a value to the variable ``image'' specifying the image to be printed on paper as given by the functions $fx$ and $fy$.
...
\noindent
\quad setmarks: \{$i \coloneq 0$; \textbf{while} $i < 1000$ \textbf{do} \{add mark; $i$ plus 1\}\};
...
\noindent
\quad add mark: \{$x \coloneq fx(i)$; $y \coloneq fy(i)$; mark pos\};
...
\noindent
\quad lineprint: \{k $\coloneq$ 0; while $k < f$ \textbf{do} \{PRSYM(sym[$k$]); $k$ \textbf{plus} 1\}; NLCR\};
