...
\noindent
in which ``perhaps still:equal $=$ EQUAL$_N$'' stands for some sort of still open primitive. When this is evaluated
$$
\text{equal} = \text{EQUAL}_j
$$
...
\noindent
1a $=$ ``\textbf{integer array} $p[l : 1000]$"
...
\noindent
\textbf{begin} $p[1] \coloneq 2$; $p[2] \coloneq 3$; $p[3] \coloneq S$; $p[4] \coloneq 7$; $p[5] \coloneq 11$; \dots \textbf{end}
...
\noindent
\quad\quad $k \coloneq k + l$; $p[k]:=j$ \textbf{end}
...
\noindent
\textbf{begin integer} $k$, $j$; $p[1] \coloneq 2$; $k \coloneq l$;
...
\noindent
\quad\textbf{while} $n <$ ord and $j$prime \textbf{do}
...
\noindent
\quad ``maker equal to remainder of $j$ over $p[n]$'';
