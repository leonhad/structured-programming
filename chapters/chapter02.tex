...
\textit{Note}:\quad $x.k_1$ is undefined for $l \neq i.$
...
\begin{enumerate}[leftmargin=2\parindent, label=(\arabic*)]
	...
	\item $(x \lor T(a))$ \textbf{down} $1 = (x$ down $1\ \lor$
	
	\quad \textbf{if} $a \neq T_0$.min \textbf{then} $T($pred$(a))$ \textbf{else} $T()$
\end{enumerate}

\tabto*{0.9em}(27)--(28) \textbf{up} is similarly defined, with succ for pred and max for min.
